...
if (isset($_POST['submit'])) {
    ...
    if ($uploadOk == 1) {
        ...
        $sql = "SELECT * FROM arbitrator WHERE email='$email'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            echo '<script>
                alert("This user already exists. Enter another email.");
                window.location.href="admin_arbitrator.php";
            </script>';
        } else {
            // Insert into database
            $insert_sql = "INSERT INTO arbitrator (name, email, phone, experience, qualification, profession, status, pic) 
...
<head>
    ...
    <title>Add New Arbitrator</title>
