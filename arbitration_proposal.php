...
if (isset($_POST['submit1'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        echo '<script>alert("Invalid form submission. Please refresh and try again."); window.location.href="arbitration_proposal.php";</script>';
        exit();
    }
    ...
    if (
        ...
        echo '<script>alert("All fields are required."); window.location.href="arbitration_proposal.php";</script>';
        exit();
    }

    // ✅ Email format validation
    if (!filter_var($email1, FILTER_VALIDATE_EMAIL) || !filter_var($email2, FILTER_VALIDATE_EMAIL)) {
        echo '<script>alert("Invalid email format."); window.location.href="arbitration_proposal.php";</script>';
        exit();
    }

    // ✅ Phone number basic validation
    if (!preg_match("/^[0-9+\-\s]{6,15}$/", $phone1) || !preg_match("/^[0-9+\-\s]{6,15}$/", $phone2)) {
        echo '<script>alert("Invalid phone number format."); window.location.href="arbitration_proposal.php";</script>';
        exit();
    }

    // ✅ Use prepared statements to prevent SQL injection
    $stmt = $conn->prepare("INSERT INTO arbitration_proposal 
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
        echo '<script>alert("Form has been submitted successfully!"); window.location.href="arbitration.php";</script>';
    } else {
        echo '<script>alert("Error submitting form. Please try again later."); window.location.href="arbitration_proposal.php";</script>';
    }
